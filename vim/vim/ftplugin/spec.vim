...
" 10/08/2001
" Modified by Sir Raorn <raorn@altlinux.ru>
...
if !exists("no_plugin_maps") && !exists("no_spec_maps")
	...
	if !hasmapto("<Plug>AddChangelogEntry")
		map <buffer> <LocalLeader>cj <Plug>AddChangelogEntry
	endif
...
if !exists("*s:InsertChangelogHeader")
	...
	function s:InsertChangelogHeader(line)
		...
		" get values
		let rpm_packager = <SID>GetExternalMacroValue("Packager")
		if (rpm_packager == "")
			let rpm_packager = <SID>GetTagValue("Packager")
		endif
		let rpm_serial = <SID>GetTagValue("Serial")
		if (rpm_serial == "")
			let rpm_serial = <SID>GetTagValue("Epoch")
		endif
		let rpm_version = <SID>GetTagValue("Version")
		let rpm_release = <SID>GetTagValue("Release")

		if (rpm_serial == "")
			let rpm_pkg_version = rpm_version . "-" . rpm_release
		else
			let rpm_pkg_version = rpm_serial . ":" . rpm_version . "-" . rpm_release
		endif

		" insert blank line first
		call append(a:line, "")
		" insert changelog header
		call append(a:line, "* " . strftime("%a %b %d %Y") . " " . rpm_packager . " " . rpm_pkg_version)
	endfunction
endif
...
if !exists("*s:GetTagValue")
	...
	function s:GetTagValue(tag)
		...
		endwhile
		" %macro
		while (value =~ '%[A-Za-z_0-9]\+')
			let macro = matchstr(value, '%[A-Za-z_0-9]\+')
			let macro_name = substitute(macro, '%\([A-Za-z_0-9]\+\)', '\1', "")
			let macro_value = <SID>GetMacroValue(macro_name)
			let value = substitute(value, '%' . macro_name, macro_value, "")
		endwhile
